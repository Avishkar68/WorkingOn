    ...
    <PageShell
      ...
      actions={
        <motion.button
          ...
          className="w-full md:w-auto btn-primary rounded-2xl px-5 py-2.5 text-sm font-bold flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(20,184,166,0.3)]"
        >
          <Plus size={18} />
      ...
      <div className="space-y-6 md:space-y-8">
        ...
        <div className="space-y-6 md:space-y-8">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 -mx-4 px-4 md:mx-0 md:px-0 scrollbar-hide">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`whitespace-nowrap flex items-center gap-2 px-4 py-2.5 rounded-2xl text-xs font-bold transition-all border shrink-0 ${activeTab === tab.id
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 -mx-4 px-4 md:mx-0 md:px-0 scrollbar-hide">
              <span className="text-[10px] font-bold uppercase tracking-widest text-text-muted mr-2 shrink-0">Sort By:</span>
              {sortTabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveSort(tab.id)}
                  className={`whitespace-nowrap flex items-center gap-2 px-3 py-1.5 rounded-xl text-[10px] font-bold transition-all border shrink-0 ${activeSort === tab.id
            ...
            <p className="text-[10px] md:text-[11px] text-text-muted/60 font-medium px-1 leading-relaxed">
          ...
          {loading ? (
            ...
            <motion.div
              ...
              initial="hidden"
              animate="visible"
              // Responsive masonry layout: 1 col on mobile, 2 on medium, 3 on xl
              className="columns-1 md:columns-2 xl:columns-3 gap-4 lg:gap-6 space-y-4 md:space-y-0"
            >
              ...
              {filteredPosts.length === 0 && (
                <div className="w-full py-20 flex flex-col items-center justify-center text-slate-500 space-y-4 text-center">
                  <div className="h-16 w-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center">
                    <Zap size={32} className="opacity-20" />
                  </div>
                  <div className="space-y-1">
                    <p className="font-bold text-lg tracking-tight">No pulses yet</p>
                    <p className="text-sm opacity-60">Be the first to share something anonymous!</p>
                  </div>
                </div>
              )}
            </motion.div>
